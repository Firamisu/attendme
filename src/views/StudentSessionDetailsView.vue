<template>
  ...
  >
    <div class="mb-4 flex flex-wrap gap-4 items-center justify-between">
      <button
        @click="goBack"
        class="border-2 border-black px-4 py-2 font-bold uppercase hover:bg-black hover:text-white"
      >
        ← Back to Sessions
      </button>
    </div>

    <div v-if="error" class="border-2 border-red-600 p-4 text-red-600">
    ...
    <div v-else>
      ...
      <div
        v-if="sessionDetails"
        class="mt-4 border-2 border-black p-4 bg-gray-50"
      >
        ...
        <p
          v-if="presentLabel"
          class="mt-3 inline-flex items-center px-3 py-1.5 text-base font-bold"
          :class="presentClass"
        >
          {{ presentLabel }}
        </p>
      ...
      <div class="mt-8">
        ...
        <div
          ...
          class="border-2 border-black p-10 text-center"
        >
          <p class="text-gray-400 font-bold">
            No attendance records for this course group.
          </p>
        </div>
      ...
      <div class="mt-8 border-2 border-black p-4 bg-gray-50">
        <h2 class="text-2xl font-bold mb-4">Attendance Progress</h2>
        <p class="text-sm text-gray-700 mb-2">
          Attended: {{ attendedSessionsCount }} / {{ totalSessionsCount }}
        </p>
        <div
          class="w-full h-4 border-2 border-black rounded-full bg-white overflow-hidden"
        >
          <div
...
async function fetchSessionDetails() {
  ...
  try {
    ...
    const match =
      allSessions.value.find((s) => s.courseSessionId === sessionId) || null;
...
const presentClass = computed(() => {
  ...
  return present ? "bg-green-500 text-white" : "bg-red-500 text-white";
});
